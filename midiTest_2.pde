...
void loop()
{
  ...
  for(int i = maxDigIn; i > minDigIn-1; i--)
  {
    ...
    // if currently not conducting and previous state was conducting, noteOn!
    if( curState[i] == LOW && state[i] == HIGH ) // x connected to foil, thus conducting +5v
      //noteOn on channel 1 (0x90), note value, middle velocity (0x45):
      noteOn(0x90, note[i], 0x45);
    // if currently conducting and previous state was not conducting, noteOff!
    if( curState[i] == HIGH && state[i] == LOW ) // x separated from foil, thus insulated
